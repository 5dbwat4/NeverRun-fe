<template>
  <section :class="$style['first-time-page']">
    <header :class="$style['heading-row']">
      <h2 :class="$style.heading">First Time Setup</h2>
    </header>

    <UCard v-if="!verifiedUser">
      <p :class="$style.intro">Welcome to NeverRun. Seems like you are first time use it. You need to configure your personal information to continue.</p>

      <UTabs :items="tabs" class="w-full">
        <template #password>
          <UCard>
            <p :class="$style['tab-desc']">Please input your ZJUAM username and password. Those data will be sent to the server for authentication only and will not be stored.</p>
            <UForm :class="$style['setup-form']" :state="passwordState" :validate="validatePassword" :validate-on="['blur']" @submit="savePassword">
        ...
        <template #token>
          <UCard>
            <p :class="$style['tab-desc']">
              ...
              <br/>
              <a href="https://zjuam.zju.edu.cn/cas/login?service=http://www.tyys.zju.edu.cn/cgapp-server/cas/appLogin" target="_blank" rel="noopener" class="text-blue-500! dark:text-blue-400! underline decoration-dashed underline-offset-4 hover:text-blue-600! dark:hover:text-blue-300!">
                https://zjuam.zju.edu.cn/cas/login?service=http://www.tyys.zju.edu.cn/cgapp-server/cas/appLogin
                <UIcon name="i-lucide-external-link" :class="$style['external-icon']" />
              </a>
              <br />
            ...
            <UForm :class="$style['setup-form']" :state="tokenState" :validate="validateToken" :validate-on="['blur']" @submit="saveToken">
      ...
      <p class="mt-4 text-[0.83rem] text-ns-muted/75 font-dm-sans">Your credentials will be sent to the server, and we will NOT store them.</p>
    </UCard>

    <UCard v-if="verifiedUser">
      <template #header>
        <h3 :class="$style['card-title']">Is it you?</h3>
      </template>

      <p :class="$style['confirm-desc']">Then everything you provided seems OK to us.</p>

      <div :class="$style['info-table']">
        <div :class="$style['info-row']">
          <span :class="$style['info-label']">xh</span>
          <span :class="$style['info-value']">{{ verifiedUser.xh }}</span>
        </div>
        <div :class="$style['info-row']">
          <span :class="$style['info-label']">xm</span>
          <span :class="$style['info-value']">{{ verifiedUser.xm }}</span>
        </div>
        <div :class="$style['info-row']">
          <span :class="$style['info-label']">xymc</span>
          <span :class="$style['info-value']">{{ verifiedUser.xymc }}</span>
        </div>
      </div>
